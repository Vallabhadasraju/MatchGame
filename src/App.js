...
import ImageToMatch from './components/ImageToMatch'
...
const App = () => {
  ...
  const handleThumbnailClick = id => {
    if (id === matchImage.id) {
      ...
      setMatchImage(imagesList[randomIndex])
    } else {
      setIsGameInProgress(false)
    }
  }
  ...
  return (
    <div className="app-container">
      ...
      {isGameInProgress ? (
        <>
          <ImageToMatch imageUrl={matchImage.imageUrl} />
          <TabsList
            tabsList={tabsList}
        ...
        <ScoreCard score={score} onPlayAgain={resetGame} />
      )}
    </div>
  )
}
